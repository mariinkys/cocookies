...
const emit = defineEmits<{ created: [category: CategoryResponse] }>()
...
const model = ref<CategoryRequest>({ name: '', description: null })
const initialValues: CategoryRequest = { name: '', description: null }
...
async function onSubmit({ valid }: FormSubmitEvent) {
  ...
  try {
    ...
    toast.add({
      ...
      detail: t('categories.messages.createError'),
      life: 3000,
    })
  } finally {
    loading.value = false
...
<template>
  <Dialog
    ...
    :header="t('categories.dialog.title')"
    :style="{ width: '24rem' }"
    modal
    :draggable="false"
  >
